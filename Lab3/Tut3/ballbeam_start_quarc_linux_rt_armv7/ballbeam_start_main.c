...
typedef RT_MODEL_ballbeam_start_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * ballbeam_start(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(ballbeam_start_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv,
    "tcpip://130.113.11.32:17001?nagle=no");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: ballbeam_start -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
  ...
  if (rtExtModeQuarcStartup(rtmGetRTWExtModeInfo(S),
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      fflush(stdout);
      result = hil_task_start(ballbeam_start_DW.HILReadTimebase_Task, (t_clock)
        ballbeam_start_P.HILReadTimebase_Clock, 500.0, -1)
        ;
      if (result == 0) {
        ...
        /* disarm the timebase */
        hil_task_stop(ballbeam_start_DW.HILReadTimebase_Task)
          ;
